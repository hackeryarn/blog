...
keywords:
  ...
  - returns
---
Django is a great framework for web development. Unfortunately, due to
...
Throughout this series we will follow the [official Django
tutorial](https://docs.djangoproject.com/en/4.0/intro/tutorial01/) while
making the code safer and easier to reason about. We will accomplish
this by enlist the help of [mypy](http://mypy-lang.org/) (for general
type checking) and [returns](https://github.com/dry-python/returns) (for
containers that will provide safety).

I will not cover the basics of Django or Python. If you ever feel like
...
to the parts of the official tutorial.
...
There are two properties that make a piece of code safe. It has to be
**pure** and **complete**.
...
We can, of course, read the source code to answer these questions. This
...
into trouble -- see the questions above. So what\'s the solution?
...
This question becomes harder to answer and reason about. This function
...
knowing without reading the body.
...
The final question is especially troubling in Python. Python a lot of
...
applications -- if not handled properly.
...
Use your favorite method to install Django, and let\'s dive in right
where part [the official
...
Next we need to install some dependencies that will allow us to use the
type system to its fullest:

- `mypy` provides our type system and integrates well with the rest of
  our dependencies.
...
The index view function will look almost the same, except we will add
some types.
...
After these steps, we can access the site at
<http://localhost:8000/polls/> after running:
...
2](https://hackeryarn.com/post/django-returns-2/), we will actually use
